    ...
    for motif in binding_motif:
        try:
            ...
            if is_present:
                ...
                # Filter every other node from matched_nodes and map to graph_nodes
                if graph:
                    graph_nodes = list(graph.nodes)  # Ensure consistent graph node order
                    print(f"Graph nodes: {graph_nodes}")

                    # Filter every second node (0 and 2) from matched_nodes and map to graph_nodes
                    aligned_nodes = [graph_nodes[index] for index in matched_nodes[::2] if index < len(graph_nodes)]
                    print(f"Aligned nodes: {aligned_nodes}")

                    # Align matched_nodes to graph_nodes sequentially
                    #aligned_nodes = graph_nodes[:len(matched_nodes)]

                    # Process the aligned nodes
                    for graph_node in aligned_nodes:
                        print(f"Processing graph node: {graph_node}")

                        if graph_node in graph.nodes:  # Ensure node exists in graph
    ...
    for glycan in glycans_no_branches:
        ...
        try:
            ...
            if matching_monosaccharides:
                ...
                print(" ")
                print("lectin: ", lectin)
                print(f"Processing glycan: {matched_glycan}")
...
# write std out put file to a file
